...
class RPN:
    ...
    def build(self, rgb):
        ...
        self.gamma3 = tf.constant(self.rpn_params['gamma3:0'], dtype=tf.float32,
                                  name='gamma3')
        self.gamma4 = tf.constant(self.rpn_params['gamma4:0'], dtype=tf.float32,
                                  name='gamma4')
        # Pooling to the same size
        self.pool3_p = tf.nn.max_pool(self.pool3, ksize=[1, 2, 2, 1],
                                      strides=[1, 2, 2, 1], padding='SAME',
                                      name='pool3_proposal')
        # L2 Normalization
        self.pool3_p = self.pool3_p / (
            tf.sqrt(tf.reduce_mean(
                tf.square(self.pool3_p))) / normalization_factor) * self.gamma3
        self.pool4_p = self.pool4 / (
            tf.sqrt(tf.reduce_mean(
                tf.square(self.pool4))) / normalization_factor) * self.gamma4
        # Proposal Convolution

        self.conv_proposal_3 = self.conv_layer(self.pool3_p, 'conv_proposal_3',
                                               use_relu=0)
        self.relu_proposal_3 = tf.nn.relu(self.conv_proposal_3)
        self.conv_proposal_4 = self.conv_layer(self.pool4_p, 'conv_proposal_4',
                                               use_relu=0)
        self.relu_proposal_4 = tf.nn.relu(self.conv_proposal_4)
        self.conv_proposal_5 = self.conv_layer(self.conv5_3, 'conv_proposal_5',
                                               use_relu=0)
        self.relu_proposal_5 = tf.nn.relu(self.conv_proposal_5)
        # Concatrate
        print(self.relu_proposal_3.shape)
        print(self.relu_proposal_4.shape)
        print(self.relu_proposal_5.shape)
        self.relu_proposal_all = tf.concat(
            [self.relu_proposal_3, self.relu_proposal_4, self.relu_proposal_5],
            3)
        print("#####", self.relu_proposal_all.shape)
        # RPN_TEST_6(>=7)

        self.conv_cls_score = self.conv_layer(self.relu_proposal_all,
                                              'conv_cls_score', use_relu=0)
        self.conv_bbox_pred = self.conv_layer(self.relu_proposal_all,
                                              'conv_bbox_pred', use_relu=0)

        assert self.conv_cls_score.get_shape().as_list()[1:] == [feature_height,
                                                                 feature_width,
                                                                 18]
        assert self.conv_bbox_pred.get_shape().as_list()[1:] == [feature_height,
                                                                 feature_width,
                                                                 36]
...
if __name__ == '__main__':
    ...
    modelPath = g_RpnPlus_Config.rpnplus_model_path
    vggModelPath = g_RpnPlus_Config.vgg16_model_path

    imageDir = g_RpnPlus_Config.image_input_dir
    resultsDir = g_RpnPlus_Config.image_save_dir
    ...
    with tf.device(gpuNow):
        ...
        for imageName in imageNames:
            ...
            pix = np.array(im.getdata()).reshape(1, image_height, image_width,
                                                 3).astype(np.float32)

            start_ = datetime.utcnow()
            [test_prob, test_bbox_pred] = sess.run([cnn.prob, cnn.bbox_pred],
                                                   feed_dict={image: pix})

            end_ = datetime.utcnow()
            c = (end_ - start_)
            print('%s uses %d milliseconds' % (
            imageName[0], c.microseconds / 1000))
