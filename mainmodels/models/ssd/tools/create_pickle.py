...
def split_data(pkl_path, split_length=1000):
    ...
    with open(pkl_path, "rb") as handle:
        ...
        for key, val in raw_data.items():
            ...
            if cnt % split_length == 0:
                ...
                sub_save_path = "/".join(
                    pkl_path.split("/")[0:-1] + ["raw_data"] +
                    [base_name + "_" + str(cnt // split_length) + ".pkl"])
                print(sub_save_path)
                if not os.path.exists("/".join(sub_save_path.split("/")[:-1])):
                    os.makedirs("/".join(sub_save_path.split("/")[:-1]))
                with open(sub_save_path, "wb") as save_handle:
                    pkl.dump(sub_raw_data, save_handle)
                sub_raw_data = dict()
        if sub_raw_data:
            base_name = ".".join(
                os.path.basename(pkl_path).split(".")[0:-1])
            sub_save_path = "/".join(
                    pkl_path.split("/")[0:-1] + ["raw_data"] +
                    [base_name + "_" + str(cnt // split_length + 1) + ".pkl"])
            with open(sub_save_path, "wb") as save_handle:
                pkl.dump(sub_raw_data, save_handle)
...
def prepare_raw_data_with_resized_NWPU(tt100k_annotation_path):
    ...
    print("Start deal with %s ..." % tt100k_annotation_path)
    with open(tt100k_annotation_path, "rb") as handle:
        test_annotation_json = pkl.load(handle)
        for abs_file_path, target_pos_list in test_annotation_json.items():
            image = Image.open(abs_file_path)
            orig_w, orig_h = image.size
            if g_SSDConfig.NUM_CHANNELS == 1:
                image = image.convert('L')  # 8-bit grayscale
            ...
            resize_image_file_path = "/".join(
                    [resized_image_save_dir,
                     "_".join(abs_file_path.split("/")[-3:]).replace(" ", "_")])
            image.save(resize_image_file_path)
            ...
            box_coords_list = list()
            for target_pos_dict in target_pos_list:
                if target_pos_dict["class"] in filter_label_list:
                    continue
                if target_pos_dict["class"] == 8:
                    target_pos_dict["class"] = 7
                elif target_pos_dict["class"] == 9:
                    target_pos_dict["class"] = 8
                else:
                    pass
                cell_dict = dict()
                cell_dict["class"] = target_pos_dict["class"]
                ulc_x, ulc_y, lrc_x, lrc_y = target_pos_dict["box_coords"]
                new_box_coords = (
                    ulc_x * x_scale, ulc_y * y_scale, lrc_x * x_scale,
                    lrc_y * y_scale)
                new_box_coords = [round(x) for x in new_box_coords]
...
if __name__ == '__main__':
    ...
    if g_SSDConfig.MODEL == "AlexNet":
        ...
        prepare_raw_data_with_resized_NWPU(g_SSDConfig.nwpu_train_annotation_path)
        split_data(g_SSDConfig.TRAIN_DATA_RAW_PATH)
    else:
